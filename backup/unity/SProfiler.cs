...
public class SProfiler : MonoSingleton<SProfiler>
{
	...
	public class ProfileObj
	{
		...
		public void PostLoad()
		{
			...
			using (Dictionary<int, SProfiler.Sample>.ValueCollection.Enumerator enumerator = this.samples.Values.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					SProfiler.Sample current = enumerator.Current;
			...
			for (int i = 0; i < SProfiler.ProfileObj.column_props.Length; i++)
			{
				...
				if (!columnProp.showInCompareMode || this.compareMode)
				{
					if (!this.compareMode || !columnProp.hidden)
					{
						...
						GUI.Box(new Rect(num, num2, (float)columnProp.width, num3), string.Empty);
						GUI.Label(new Rect(num + 4f, num2, (float)(columnProp.width - 8), num3), columnProp.data);
		...
		private void DrawButtons()
		{
			...
			float num4 = 2f;
			GUI.color = Color.green;
			if (GUI.Button(new Rect(num4, num3, 88f, num), "MODE"))
			{
				SProfiler.showMaxOnly = !SProfiler.showMaxOnly;
				this.Reset();
			}
			num3 += num + num2;
			if (GUI.Button(new Rect(num4, num3, 88f, num), "RESET"))
			{
				this.Cleanup();
			}
			num3 += num + num2;
			...
			if (GUI.Button(new Rect(num4, num3, 88f, num), "PAUSE"))
			{
			...
			if (GUI.Button(new Rect(num4, num3, 88f, num), "SHOW"))
			{
			...
			if (GUI.Button(new Rect(num4, num3, 88f, num), "EVENT SYS"))
			{
			...
			num3 += num + num2;
			if (GUI.Button(new Rect(num4, num3, 88f, num), "SAVE"))
			{
				this.SaveProfileData();
			}
			num3 += num + num2;
			if (GUI.Button(new Rect(num4, num3, 88f, num), "LOAD"))
			{
				this.RefreshProfileFiles();
				this.showProfileFiles = true;
			}
			num3 += num + num2;
			if (GUI.Button(new Rect(num4, num3, 88f, num), "COMPARE"))
			{
				this.RefreshProfileFiles();
				this.showProfileFiles = true;
			...
			if (GUI.Button(new Rect(num4, num3, 88f, num), "FPS_SHOW"))
			{
			...
			if (GUI.Button(new Rect(num4, num3, 88f, num), "FPS_PAUSE"))
			{
...
// 				ActorsStatistic.sampleActors = !ActorsStatistic.sampleActors;
			...
			num3 += num + num2;
			GUI.color = color;
			if (GUI.Button(new Rect(num4, num3, 88f, num), "FPS_RESET"))
			{
 				FPSStatistic.Reset();
// 				ActorsStatistic.Reset();
			...
			if (this.compareMode)
			{
				...
				if (GUI.Button(new Rect(num4, num3, 88f, num), "[TIME]"))
				{
				...
				if (GUI.Button(new Rect(num4, num3, 88f, num), "[SELF TIME]"))
				{
				...
				if (GUI.Button(new Rect(num4, num3, 88f, num), "[AVG TIME]"))
				{
				...
				if (GUI.Button(new Rect(num4, num3, 88f, num), "[AVG SELF]"))
				{
				...
				if (GUI.Button(new Rect(num4, num3, 88f, num), "[MAX TIME]"))
				{
				...
				if (GUI.Button(new Rect(num4, num3, 88f, num), "[MAX SELF]"))
				{
				...
				if (GUI.Button(new Rect(num4, num3, 88f, num), "[COUNT]"))
				{
		...
		public void SaveProfileData()
		{
			...
			string path = string.Format("{0}/{1}.spf", "d:"/*DebugHelper.logRootPath*/, text);
		...
		private void DrawProfileFiles()
		{
			...
			if (this.profileFiles != null && this.profileFiles.Length > 0)
			{
				...
				for (int i = 0; i < this.profileFiles.Length; i++)
				{
					...
					int num7 = Mathf.Max(text.LastIndexOf('/'), text.LastIndexOf('\\'));
					text = text.Substring(num7 + 1);
					if (GUI.Button(new Rect(num2, num, num4, num3), text))
					{
						num6 = i;
					}
					num += num3 + num5;
				...
				{
					this.showProfileFiles = false;
					SProfiler.ProfileObj profileObj = SProfiler.ProfileObj.LoadFromFile(this.profileFiles[num6]);
					if (this.compareMode)
					{
						if (profileObj == null)
						{
		...
		private void RefreshProfileFiles()
		{
            ...
            string logRootPath = "d:/";// DebugHelper.logRootPath;
		...
		public void DrawGroups_MaxSelfTimeOnly()
		{
			...
			GUI.color = Color.green;
			float num4 = 80f;
			if (GUI.Button(new Rect(num, 0f, num4, num2), "RESET"))
			{
				this.Cleanup();
			}
			num += num4 + 10f;
			if (GUI.Button(new Rect(num, 0f, num4, num2), "MODE"))
			{
				SProfiler.showMaxOnly = !SProfiler.showMaxOnly;
				this.Cleanup();
			}
			...
			if (GUI.Button(new Rect(num, 0f, num4, num2), "PAUSE"))
			{
				SProfiler.paused = !SProfiler.paused;
			}
			GUI.color = Color.yellow;
			GUI.Label(new Rect(0f, num2, (float)Screen.width, num3), text);
			GUI.color = Color.white;
		}
		...
		public void SaveToFile(string path)
		{
			...
			try
			{
				...
				fileStream = new FileStream(path, FileMode.Create, FileAccess.Write, 0);
	...
	private void OnGUI()
	{
		...
		{
            if (GUI.Button(new Rect(0, 0, 100, 50), "Profiler"))
            {
                SProfiler.StartProfile();
            }
			return;
